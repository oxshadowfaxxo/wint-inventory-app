  ...
  return (
    <s-box padding="base" borderWidth="base" borderRadius="base">
      <s-stack direction="block" gap="small">
        ...
        <s-text>SKU: {result.sku || "—"} · Barcode: {result.barcode || "—"}</s-text>
        ...
        <div style={{ marginTop: 16 }}>
          <fetcher.Form method="post">
            ...
            <label style={{ display: "grid", gap: 6, maxWidth: 560 }}>
              ...
              <input name="search" placeholder="Search by barcode, SKU, or product title" required />
            </label>
            <div style={{ marginTop: 12 }}><s-button type="submit" disabled={fetcher.state !== "idle"}>Search Shopify</s-button></div>
          </fetcher.Form>
          {fetcher.data?.error && <s-banner tone="critical">{fetcher.data.error}</s-banner>}
          {fetcher.data?.searched && results.length === 0 && !fetcher.data?.error && (
            <s-paragraph>No Shopify products matched your search.</s-paragraph>
          )}
          {results.length > 0 && (
            <s-stack direction="block" gap="base">
              {results.map((result) => (
                <AddProductResult key={result.variantId} countId={countId} result={result} alreadyInCount={existingVariantIds.includes(result.variantId)} />
              ))}
            </s-stack>
          )}
        </div>

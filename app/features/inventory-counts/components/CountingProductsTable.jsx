...
import { ScanOrSearchInput } from "./ScanOrSearchInput";
...
export function CountingProductsTable({ countId, lines }) {
  ...
  const { sort, sortedLines, toggleSort, resetSort } =
    useInventoryCountSort(lines);
  const selectedLines = useMemo(
    () => lines.filter((line) => selectedIds.has(line.id)),
    [lines, selectedIds],
  );
  const visibleSelectedCount = sortedLines.filter((line) =>
  ...
  const handleLocateLine = useCallback((lineId) => {
    window.clearTimeout(highlightTimer.current);
    window.requestAnimationFrame(() => {
      window.requestAnimationFrame(() => {
        const row = document.getElementById(`count-line-${lineId}`);
        if (!row) return;
        row.scrollIntoView({ behavior: "smooth", block: "center" });
        row.classList.add(styles.recentlyScanned);
        highlightTimer.current = window.setTimeout(
          () => row.classList.remove(styles.recentlyScanned),
          1800,
        );
      });
    });
  ...
  function toggleAllVisible() {
    setSelectedIds((current) => {
      ...
      if (allVisibleSelected)
        sortedLines.forEach((line) => next.delete(line.id));
  ...
  return (
    <s-stack direction="block" gap="base">
      ...
      {confirming && (
        <s-section
          heading={`Remove ${selectedIds.size} ${selectedIds.size === 1 ? "product" : "products"} from this count?`}
        >
          <s-paragraph>
            This removes the selected products and their counted quantities from
            the count sheet. It does not change Shopify inventory.
          </s-paragraph>
          <s-unordered-list>
            {selectedLines.slice(0, 5).map((line) => (
              <s-list-item key={line.id}>
                {line.productTitle}
                {line.variantTitle ? ` — ${line.variantTitle}` : ""} — Counted:{" "}
                {line.countedQuantity}
              </s-list-item>
            ))}
          </s-unordered-list>
          {selectedLines.length > 5 && (
            <s-paragraph>And {selectedLines.length - 5} more</s-paragraph>
          )}
          ...
          <fetcher.Form method="post">
            ...
            <s-stack direction="inline" gap="base">
              <s-button type="button" onClick={() => setConfirming(false)}>
                Keep Products
              </s-button>
              <s-button
                tone="critical"
                type="submit"
                disabled={busy || wouldRemoveEveryLine}
              >
                Remove Products
              </s-button>
            </s-stack>
          ...
          <thead>
            <tr>
              ...
              <SortableTableHeader
                ...
                onSort={toggleSort}
                className={
                  selectionMode ? styles.productWithSelection : undefined
                }
              />
              <SortableTableHeader
                column="variant"
                  ...
                  {selectionMode && (
                    <td className={styles.selectionCell}>
                      <input
                        ...
                        aria-label={`Select ${line.productTitle}${line.variantTitle ? ` — ${line.variantTitle}` : ""}`}
                      />
                    </td>
                  )}
                  <td
                    className={`${styles.product} ${selectionMode ? styles.productWithSelection : ""}`}
                  >
                    {line.productTitle}
                  </td>
